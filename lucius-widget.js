(function () {
  ...
  function initLucius() {
    ...
    if (!mount) {
      console.warn("Lucius: #lucius-embed not found. Retrying…");
      setTimeout(initLucius, 200);
      return;
    }
    ...
    function appendMessage(cls, text) {
      ...
      body.insertAdjacentHTML(
        "beforeend",
        `<div class="lw-msg ${cls}">${escapeHtml(text)}</div>`
      );
    ...
    function render() {
        ...
        <div class="lw-embed-shell">
          ...
          <div class="lw-body" id="lw-body">
            <div class="lw-msg sys">
              Hi — I’m Lucius. Ask me about System Designer (Distributed Systems),
              Revit support (2022–2026), and early access.
              For enterprise or manufacturer discussions, email
              <b>info@bimacoustics.net</b>.
            </div>
            ...
            ${isGated ? `
              <div class="lw-consent">
                ...
                <div class="lw-consent-actions">
                  <button class="lw-btn" id="lw-consent-no">No thanks</button>
                  <button class="lw-btn primary" id="lw-consent-yes" disabled>Continue</button>
                </div>
              </div>
            ` : ""}
          </div>
          ...
          <div class="lw-ft">
            <input
              ...
              ${isGated ? "disabled" : ""}
            />
            <button class="lw-send" id="lw-send" ${isGated ? "disabled" : ""}>
              ${state.busy ? "…" : "Send"}
            </button>
          </div>
        </div>
      ...
      if (check && yesBtn) {
        check.onchange = () => yesBtn.disabled = !check.checked;
      }
    ...
    async function sendMessage() {
      ...
      try {
        ...
        appendMessage("bot", data.reply || "No response available.");
      } catch (e) {
        console.error("Lucius fetch failed:", e);
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initLucius);
  } else {
    initLucius();
  }
